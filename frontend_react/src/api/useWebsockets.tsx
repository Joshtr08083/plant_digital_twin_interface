...
export function useWebsockets(windowSeconds: number = 5) {
    ...
    const [fire] = useState<boolean>(true);
        ...
        socket.onmessage = (event: MessageEvent<string>) => {
            const reading: Reading = JSON.parse(event.data);
                ...
                if (reading.status === 200 && reading.data !== null) {
                    setStatus((prev) => (prev.ok === true && prev.message === 'Live') ? prev : {ok: true, message: 'Live'});
                    setDataPoints((prev) => {
                    ...
                    const msg = `Sensor error (${reading.status}): ${reading.error}`;
                    setStatus((prev) => (prev.ok === false && prev.message === msg) ? prev : { ok: false, message: msg });
                }
        }
